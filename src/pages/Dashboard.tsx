...
export default function Dashboard() {
  ...
  (certificates || []).forEach(c => {
    ...
    if (days <= 0) {
      criticalItems.push({
        ...
        link: `/vehicles`,
        severity: Math.abs(days),
      });
    }
  });

  // Vehicles with negative km_until_service
  (vehicles || []).forEach(v => {
    ...
    if (kmUntil < 0) {
      criticalItems.push({
        ...
        link: `/vehicles/${v.id}`,
        severity: Math.abs(kmUntil),
      });
    }
  });

  // Drivers with expired licence/PrDP
  (drivers || []).forEach(d => {
    ...
    if (licDays !== null && licDays <= 0) {
      criticalItems.push({
        ...
        detail: `${d.full_name} — ${Math.abs(licDays)}d overdue`,
        link: `/drivers/${d.id}`,
        severity: Math.abs(licDays),
      });
    }
    if (prdpDays !== null && prdpDays <= 0) {
      criticalItems.push({
        type: "driver",
        label: `Expired PrDP`,
        detail: `${d.full_name} — ${Math.abs(prdpDays)}d overdue`,
        link: `/drivers/${d.id}`,
        severity: Math.abs(prdpDays),
      });
    }
  });
  ...
  // Expiring certs (within 30 days, not yet expired)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Detail Panel */}
      {activePanel && (
        <div className="stat-card border-primary/30 animate-in fade-in-0 slide-in-from-top-2 duration-200">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {criticalItems.length === 0 ? (
                ...
                <div key={i} className="flex items-center justify-between py-2 px-3 bg-destructive/5 rounded-lg border border-destructive/20">
                  ...
                  <button onClick={() => navigate(item.link)} className="text-xs text-primary hover:underline flex items-center gap-1">
                ...
                <div key={d.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/30">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold px-2 py-1 rounded-full uppercase ${
                      d.driverStatus === "critical" ? "bg-destructive/20 text-destructive" :
                      d.driverStatus === "warning" ? "bg-warning/20 text-warning" :
                ...
                <div key={v.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/30">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {v.kmUntil < 0 ? `Service overdue by ${Math.abs(v.kmUntil).toLocaleString()} km` : `${v.kmUntil.toLocaleString()} km until service`}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold px-2 py-1 rounded-full uppercase ${
                      v.effectiveStatus === "critical" ? "bg-destructive/20 text-destructive" :
                      v.effectiveStatus === "warning" ? "bg-warning/20 text-warning" :
                      "bg-muted/20 text-muted-foreground"
                    }`}>{v.effectiveStatus}</span>
              ...
              {expiringCertsList.length === 0 ? (
                ...
                return (
                  <div key={c.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-warning/5 border border-warning/20">
                    ...
                    <span className={`text-xs font-semibold px-2 py-1 rounded ${days <= 7 ? "bg-destructive/20 text-destructive" : "bg-warning/20 text-warning"}`}>
                      {days}d left
                    </span>
                  </div>
                );
...
function StatCard({ icon: Icon, label, value, color, highlight, onClick, active }: {
  ...
  return (
    <div
      className={`stat-card transition-all ${highlight ? "border-destructive/50" : ""} ${onClick ? "cursor-pointer hover:ring-1 hover:ring-primary/50" : ""} ${active ? "ring-2 ring-primary" : ""}`}
      onClick={onClick}
    >
