                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground font-mono">
                        {isFridgeOnly
                          ? ((v as any).customer_name || "No customer")
                          : `${v.fleet_number || "-"} · ${v.make} ${v.model}`}
                      </p>
                      {isFridgeOnly ? (
                        <p className="text-xs text-muted-foreground mt-1">{(v as any).fridge_brand} {(v as any).fridge_model}</p>
                      ) : (
                    ...
                    <>
                      <div className="grid grid-cols-3 gap-2 text-xs my-3">
                        <div>
                          ...
                          <p className={`font-mono font-semibold ${kmUntil < 0 ? "text-destructive" : kmUntil < 2000 ? "text-warning" : "text-foreground"}`}>{kmUntil.toLocaleString()}</p>
                      ...
                      <div onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => setUpdateKmFor({ id: v.id, reg: v.registration_number, km: v.current_odometer_km ?? 0 })} className="w-full inline-flex items-center justify-center gap-1.5 text-xs bg-secondary hover:bg-secondary/80 text-foreground px-3 py-2 rounded-md">
